...
object TestFetching {
  ...
  def withLookup[K, V](lookup: K => Option[V]): Fetching[K, V] = new Fetching[K, V] {
    override def fetch(key: K)(implicit ec: ExecutionContext): Future[MissingOrETagged[V]] = Future.successful {
      lookup(key).fold[MissingOrETagged[V]](Missing) { value =>
        ETaggedData(value.hashCode().toString, value)
      }
    }
    override def fetchOnlyIfETagChanged(key: K, ETag: String)(implicit ec: ExecutionContext): Future[Option[MissingOrETagged[V]]] =
      fetch(key).map {
